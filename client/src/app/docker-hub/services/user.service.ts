...
export class UserService {
  ...
  getAllUserIsMyOrganization(username: string) {
    return this.http.get(this.API_URL + 'myfriends?username=' + username)
      .pipe(
        map((res: any) => {
    ...
    searchUserByQuery(username: string) {
      return this.http.get(this.API_URL + 'search-users?username=' + username)
      .pipe(
        map((res: any) => {
          if (this._checkResponse(res)) {
            return {status: false, message: null}
